...
# ── 선반 재고 및 백오더 딕셔너리 초기화 ───────────
shelf_inventory = {}   # (row, col) 키에 남은 재고 수 저장
shelf_backorder = {}   # (row, col) 키에 백오더 수 저장
...
class Item:
   ...
   def draw(self, screen, offset_x, offset_y):
        ...
        # self.number 대신, 전역 shelf_inventory에서 남은 재고 읽어서 표시
        inv = shelf_inventory.get((self.x, self.y), 0)
        txt = bold_font.render(str(inv), True, BLACK)
        surf.blit(txt, txt.get_rect(center=surf.get_rect().center))
...
class AGV:
   ...
   def find_next_item(self):
       ...
       weight_factor = 7  # 가중치 중요도 (7:1 비율 적용)
       ...
       # 경로의 마지막 지점을 타겟으로 설정
       self.target_x, self.target_y = best_path[-1]
       self.path = best_path
    ...
    # sim_time이 정의되지 않았다면 0으로 초기화
       ...
       if not self.path:
           if self.state == "MOVING_TO_ITEM" and self.current_items:
               # 픽킹 시간: 아이템당 5초 고정
               self.action_duration = 5 * len(self.current_items)
               self.state = "PICKING"
               self.action_timer = 0

           elif self.state == "MOVING_TO_EXIT" and self.picked_items:
               # 드롭오프 시간: 아이템당 5초 고정
               self.action_duration = 5 * len(self.picked_items)
               self.state = "DROPPING"
               self.action_timer = 0
   ...
   for od in orders:
       for _ in range(od.num_items):
           if target_positions:
               ...
               # 위치별 주문 수량 누적
               location_order_counts[pos] = location_order_counts.get(pos, 0) + 1
               ...
               it.number = 5  # 모든 아이템 동일 가중치
               od.add_item(it)
               items.append(it)

   # **실제 초기 재고를 “셀별 주문 개수”로 설정**
   shelf_inventory.clear()
   shelf_backorder.clear()
   for pos, count in location_order_counts.items():
       # 최대 500개로 상한
       shelf_inventory[pos] = min(count, 500)
       # 500 초과분은 백오더로
       shelf_backorder[pos] = max(count - 500, 0)
   logging.info(f"총 {len(items)}개 아이템 생성 완료 — 셀  {len(location_order_counts)}곳") 
...
def generate_items_from_excel_data(orders_data):
   ...
   for idx, m in melted.iterrows():
       ...
       # “이 셀에 아이템 1개” 누적 → 재고 표시용
       shelf_inventory[(sim_x, sim_y)] = shelf_inventory.get((sim_x, sim_y), 0) + 1
