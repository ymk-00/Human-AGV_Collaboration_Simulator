    ...
    # sim_time이 정의되지 않았다면 0으로 초기화
       ...
       if not self.path:
           if self.state == "MOVING_TO_ITEM" and self.current_items:
                # AGV: 무거운 순(번호 큰 순)으로 피킹 시간 설정
                weight = self.current_items[0].number
                base, factor = 1.0, 0.3
                self.action_duration = base + factor * (11 - weight)
                self.state = "PICKING"
                self.action_timer = 0
...
class Human(AGV):
    ...
    def update(self, dt):
        # HUMAN: 가벼운 순으로 피킹 시간 설정
        if self.state == "MOVING_TO_ITEM" and self.current_items:
            weight = self.current_items[0].number
            base, factor = 1.0, 0.3
            self.action_duration = base + factor * weight
