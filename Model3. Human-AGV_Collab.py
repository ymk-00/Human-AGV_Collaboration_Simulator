...
class Item:
   ...
   def draw(self, screen, offset_x, offset_y):
        ...
        txt = bold_font.render(str(self.number), True, BLACK)
        surf.blit(txt, txt.get_rect(center=surf.get_rect().center))
...
class AGV:
   ...
   def find_next_item(self):
       ...
       for pos, items in location_groups.items():
           ...
           # 경로가 유효한 경우에만 처리
           if path:
               # AGV/Human 여부에 따라 다른 점수 계산 방식 적용
               if isinstance(self, Human):
                   # Human: 가벼운 아이템(낮은 번호) 선호 + 짧은 경로 선호
                   # 11-items[0].number: 가중치 값 반전 (1이 가장 높은 점수)
                   score = (11 - items[0].number) * weight_factor - len(path) - path_penalty
               else:
                   # AGV: 무거운 아이템(높은 번호) 선호 + 짧은 경로 선호
                   score = items[0].number * weight_factor - len(path) - path_penalty
    ...
    # sim_time이 정의되지 않았다면 0으로 초기화
       ...
       if not self.path:
           if self.state == "MOVING_TO_ITEM" and self.current_items:
               # AGV는 (11-number)초, Human은 number초를 unit_time으로
               if isinstance(self, Human):
                   unit_time = self.current_items[0].number
               else:
                   unit_time = 11 - self.current_items[0].number
               # 동일 위치에서 n개 픽업 시 unit_time * n
               self.action_duration = unit_time * len(self.current_items)
               self.state = "PICKING"
               self.action_timer = 0
               
           elif self.state == "MOVING_TO_EXIT" and self.picked_items:
               if isinstance(self, Human):
                   drop_time = sum(item.number for item in self.picked_items)
               else:
                   drop_time = sum((11 - item.number) for item in self.picked_items)
               self.action_duration = drop_time
               # 실제 드롭오프 단계로 진입
               self.state = "DROPPING"
               self.action_timer = 0
           elif self.state == "DROPPING":
   ...
   for od in orders:
       for _ in range(od.num_items):
           if target_positions:
               ...
               it.number = random.randint(1, 10)  # 무게 랜덤 할당
               od.add_item(it)
               items.append(it)
